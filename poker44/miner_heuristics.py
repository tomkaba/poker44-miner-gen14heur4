...
_GEN14_PROFILE: Optional[dict] = None
...
def score_chunks_gen7heur10(chunks: List[List[dict]]) -> Tuple[List[float], List[str], Dict[str, int]]:
    """Score chunks with gen7heur9 and rebalance near-threshold cases to 50/50."""
    if not chunks:
        return [], [], {
            "n_chunks": 0,
            "target_bots": 0,
    ...
    routes: List[str] = []
    for chunk in chunks:
        score, _ = score_chunk_gen7heur9(chunk)
        scores.append(float(score))
        routes.append("gen7heur10")
    ...
    if initial_bots > target_bots:
        ...
        for idx in candidates[:need]:
            ...
            routes[idx] = "gen7heur10_rebalance"
            flips += 1
...
def _load_gen14_profile() -> dict:
    global _GEN14_PROFILE
    if _GEN14_PROFILE is not None:
        return _GEN14_PROFILE
    env_path = os.getenv("POKER44_GEN14_PROFILE", "").strip()
    if env_path:
        profile_path = Path(env_path)
    else:
        profile_path = Path(__file__).resolve().parents[1] / "models" / "gen14_profile.json"
    with open(profile_path, "r", encoding="utf-8") as handle:
        _GEN14_PROFILE = json.load(handle)
    return _GEN14_PROFILE


def _extract_gen14_features(chunk: List[dict]) -> Dict[str, float]:
    action_counter: Counter = Counter()
    street_counter: Counter = Counter()
    actor_counter: Counter = Counter()
    ...
    raise_bb_values: List[float] = []
    bet_bb_values: List[float] = []
    call_bb_values: List[float] = []
    pot_values: List[float] = []
    player_counts: List[float] = []
    street_depths: List[float] = []
    stack_means: List[float] = []
    stack_stds: List[float] = []
    ...
    for hand in chunk:
        ...
        for player in players:
            ...
            if stack is None:
                continue
            try:
                stack_values.append(float(stack))
            except (TypeError, ValueError):
                continue
        stack_means.append(float(sum(stack_values) / len(stack_values)) if stack_values else 0.0)
        if len(stack_values) < 2:
            stack_stds.append(0.0)
        else:
            avg_stack = float(sum(stack_values) / len(stack_values))
            stack_stds.append(
                float(math.sqrt(sum((value - avg_stack) ** 2 for value in stack_values) / len(stack_values)))
            )
        ...
        for action in actions:
            ...
            street = str(action.get("street") or "unknown").strip().lower()
            actor = str(action.get("actor_seat") or "?")
            ...
            if atype in {"call", "check"}:
                passive_actions += 1

            if atype == "raise":
                raise_bb_values.append(amount_bb)
            elif atype == "bet":
                bet_bb_values.append(amount_bb)
            elif atype == "call":
                call_bb_values.append(amount_bb)

    hand_count = len(chunk)
    total_actions = sum(action_counter.values())
    denom = max(1, total_actions)
    aggressive_size_values = raise_bb_values + bet_bb_values
    ...
    def _sstd(vals: List[float]) -> float:
        ...
        return float(math.sqrt(sum((v - mean_val) ** 2 for v in vals) / len(vals)))

    def _smax(vals: List[float]) -> float:
        return float(max(vals)) if vals else 0.0

    def _smin(vals: List[float]) -> float:
    ...
    return {
        ...
        "raise_ratio": action_counter.get("raise", 0) / denom,
        "bet_ratio": action_counter.get("bet", 0) / denom,
        "aggression_ratio": aggressive_actions / max(1, passive_actions),
        "aggressive_action_rate": aggressive_actions / denom,
        "passive_action_rate": passive_actions / denom,
        "showdown_rate": showdown_count / max(1, hand_count),
        "street_depth_mean": _smean(street_depths),
        "street_depth_std": _sstd(street_depths),
        "players_mean": _smean(player_counts),
        ...
        "pot_mean": _smean(pot_values),
        "pot_std": _sstd(pot_values),
        "pot_max": _smax(pot_values),
        "raise_bb_mean": _smean(raise_bb_values),
        "raise_bb_std": _sstd(raise_bb_values),
        "bet_bb_mean": _smean(bet_bb_values),
        "bet_bb_std": _sstd(bet_bb_values),
        "call_bb_mean": _smean(call_bb_values),
        "aggressive_bb_mean": _smean(aggressive_size_values),
        "aggressive_bb_std": _sstd(aggressive_size_values),
        "aggressive_bb_max": _smax(aggressive_size_values),
        "aggressive_bb_min": _smin(aggressive_size_values),
        ...
        "stack_mean": _smean(stack_means),
        "stack_std_mean": _smean(stack_stds),
        "size_to_pot_ratio": _smean(aggressive_size_values) / max(_EPS_G7, _smean(pot_values)),
    }


def score_chunk_gen14heur(chunk: List[dict]) -> Tuple[float, str]:
    """Score chunk using gen14 healthy-feature logistic profile."""
    if not chunk:
        return 0.5, "gen14_empty"

    try:
        profile = _load_gen14_profile()
    except Exception:
        return 0.5, "gen14_profile_load_error"

    feature_names = profile.get("feature_names") or []
    coef = profile.get("coef")
    intercept = profile.get("intercept")
    scaler_mean = profile.get("scaler_mean") or []
    scaler_scale = profile.get("scaler_scale") or []

    if not feature_names or coef is None or intercept is None:
        return 0.5, "gen14_profile_invalid"

    features = _extract_gen14_features(chunk)
    raw = float(intercept)

    for index, name in enumerate(feature_names):
        value = float(features.get(name, 0.0))
        mean = float(scaler_mean[index]) if index < len(scaler_mean) else 0.0
        scale = float(scaler_scale[index]) if index < len(scaler_scale) else 1.0
        if abs(scale) < _EPS_G7:
            scale = 1.0
        z = (value - mean) / scale
        raw += float(coef[index]) * z

    score = _sigmoid_g7(raw)
    return round(_clamp01(score), 6), "gen14_profile"
...
def get_chunk_scorer_startup_check(scorer: str) -> Dict[str, object]:
    ...
    info: Dict[str, object] = {
        "scorer": scorer_norm,
        "active": scorer_norm in {"gen7heur10", "gen14heur1", "gen14heur2", "gen14heur3", "gen14heur"},
        "ok": True,
        "error": None,
        "details": {},
    }

    if scorer_norm in {"gen14heur1", "gen14heur2", "gen14heur3", "gen14heur"}:
        env_path = os.getenv("POKER44_GEN14_PROFILE", "")
        profile_path = (
            Path(env_path)
            if env_path
            else Path(__file__).resolve().parents[1] / "models" / "gen14_profile.json"
        )
        details = {
            "profile_path": str(profile_path),
            "profile_exists": profile_path.exists(),
            "profile_type": "gen14_logreg_profile",
        }
        ...
        try:
            ...
            has_required_keys = all(
                key in profile
                for key in ("feature_names", "coef", "intercept", "scaler_mean", "scaler_scale")
            )
            if not has_required_keys:
                info["ok"] = False
                info["error"] = "gen14 profile missing required keys"
        except Exception as exc:
            info["ok"] = False
            info["error"] = str(exc)

    if scorer_norm in {"gen7heur10"}:
        env_path = os.getenv("POKER44_GEN7HEUR9_PROFILE", "")
        profile_path = (
            Path(env_path)
            if env_path
        ...
        details = {
            "profile_path": str(profile_path),
            "profile_exists": profile_path.exists(),
            "rebalance_target": "50/50" if scorer_norm == "gen7heur10" else "disabled",
        }
        info["details"] = details
        try:
